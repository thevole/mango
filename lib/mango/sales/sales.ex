defmodule Mango.Sales do
  ...
  def add_to_cart(%Order{line_items: []} = cart, cart_parms) do
    attrs = %{line_items: [cart_parms]}
    update_cart(cart, attrs)
  end

  def add_to_cart(%Order{line_items: existing_line_items} = cart, cart_params) do
    new_item = %{product_id: String.to_integer(cart_params["product_id"]), quantity: String.to_integer(cart_params["quantity"])}
